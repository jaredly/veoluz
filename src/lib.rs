...
use nalgebra::geometry::{Isometry2, Translation2, Rotation2};
use nalgebra as na;
...
#[inline]
fn ray_arc_collision(ray: &Ray<line::float>, arc: (&Ball<line::float>, &Point2<line::float>, line::float, line::float)) -> Option<ncollide2d::query::RayIntersection<line::float>> {
    match ball_toi_with_ray(&arc.1, arc.0.radius(), &ray) {
        ...
        Some(BallResult::Inside(dist)) => {
            ...
            if is_between(place, arc.2, arc.3) {
                Some(RayIntersection::new(dist, normal, ncollide2d::shape::FeatureId::Face(0)))
            } else {
                None
            }
        }
        Some(BallResult::Outside(closer, farther)) => {
            ...
            let place = normal.y.atan2(normal.x) + std::f32::consts::PI;
            if is_between(place, arc.2, arc.3) {
                Some(RayIntersection::new(closer, normal, ncollide2d::shape::FeatureId::Face(0)))
            } else {
                ...
                let place = normal.y.atan2(normal.x) + std::f32::consts::PI;
                if is_between(place, arc.2, arc.3) {
                    Some(RayIntersection::new(farther, normal, ncollide2d::shape::FeatureId::Face(0)))
                } else {
                    None
                }
...
pub fn draw(
    ...
    for i in 0..count {
        ...
        let r0 = 50.0;
        let r1 = 200.0;
        let td = line::PI / (count as line::float / 2.0) + i as line::float * 0.1;
        walls.push(Wall::Line(ncollide2d::shape::Segment::new(
...
enum Wall {
    ...
    Circle(ncollide2d::shape::Ball<line::float>, Point2<line::float>, line::float, line::float)
}

impl Wall {
    fn toi_and_normal_with_ray(&self, ray: &Ray<line::float>) -> Option<ncollide2d::query::RayIntersection<line::float>> {
        use ncollide2d::query::ray_internal::ray::RayCast;
        match self {
            Wall::Line(wall) => wall.toi_and_normal_with_ray(&nalgebra::geometry::Isometry::identity(), ray, true),
            Wall::Circle(circle, center, t0, t1) => {
                ray_arc_collision(&ray, (circle, center, *t0, *t1))

                // if *t0 == -std::f32::consts::PI && *t1 == std::f32::consts::PI {
                //     res
                // } else {
                //     match res {
                //         None => None,
                //         Some(contact) => {
                //             // could probably do some heuristics with quadrants to avoid this some times
                //             let normal = contact.normal.y.atan2(contact.normal.x);
                //             if normal > *t0 && normal < *t1 {
                //                 Some(contact)
                //             } else {
                //                 None
                //             }
...
fn find_collision(walls: &[Wall], ray: &Ray<line::float>) -> Option<(line::float, usize, Vector2<line::float>)> {

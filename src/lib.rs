...
fn zen_photon(width: usize, height: usize) -> Vec<u8> {
    ...
    let walls = vec![((100, 100), (100, 200))];

    let mut closest = (-std::f32::INFINITY, -1);

    for _ in 0..10 {
        let direction = random::<f32>();
        let point = Point2::new(width as isize / 2, height as isize / 2);
        let mut ray = ncollide2d::query::Ray::new(point, Vector2::new(1.0, 1.0));
